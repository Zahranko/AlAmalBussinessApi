...
namespace AlAmalBusiness.Application.Services.Imp.Feedback
{
    ...
    internal static class FeedbackEmailTemplate
    {
        public static EmailMessage Build(string to, PatientFeedback feedback, string? departmentName)
        {
            ...
            html.Append("<div dir=\"rtl\" style=\"font-family:Tahoma,Arial,sans-serif;font-size:14px;color:#1f2937;\">");
            ...
            {
                html.Append("<tr>");
                html.Append($"<td style=\"font-weight:bold;border-bottom:1px solid #e5e7eb;white-space:nowrap;\">{Encode(label)}</td>");
                html.Append($"<td style=\"border-bottom:1px solid #e5e7eb;\" dir=\"auto\">{Encode(value)}</td>");
                html.Append("</tr>");
            }
            html.Append("</table>");
            html.Append("<h3 style=\"margin:16px 0 6px;font-size:15px;\">التفاصيل</h3>");
            html.Append($"<div dir=\"auto\" style=\"white-space:pre-wrap;background:#f9fafb;border:1px solid #e5e7eb;padding:10px;\">{Encode(details)}</div>");
