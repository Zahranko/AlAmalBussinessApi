...
- **No notifications** — the old app pushed to a per-department head (in-app inbox + FCM web push + a SignalR hub). None of it came across, and with it went the `HeadUserId` column that only existed to route those notifications.
